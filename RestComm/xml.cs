...
namespace RestComm
{
		...
		private static string pathfinder(XmlNode currentnode,string finalnode,string path){
			try{
			bool lastnode = false;
			path+=currentnode.LocalName+"/";

			foreach (XmlNode x in currentnode.ChildNodes) 
			{
				if (x.LocalName == finalnode) 
				{
					lastnode = true;
				}
			...
			if(!lastnode)
			{
				path = pathfinder (currentnode.FirstChild, finalnode, path);
			}
			return path;
